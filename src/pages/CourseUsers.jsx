import React , {useEffect , useState} from "react";
import Layout from "../containers/Layout";
import {useParams} from "react-router-dom";
import UserItems from "../containers/listSection/UserItems";
import SearchBar from "../containers/listSection/SearchBar";
import MenuItems from "../containers/menuSection/MenuItems";
import axios from 'axios'
import AuthService from '../auth/useAuth'

export default function CourseUsers() {
    const [terms , setTerms] = useState([])
    const [role , setRole] = useState(null)
    const {TermName}  = useParams();
    const {courseName}  = useParams();


  const token = AuthService.getCurrentUser()
  const headers = { 'test': token }
  const userRole = AuthService.getRole()

    useEffect(()=>{
        const _data = {
          cgID : "cgID" ,
          term : "term"
        }
        // axios.get(`${process.env.REACT_APP_API_URL}/term/${TermName}/registration_courses` , {headers}).then((res)=>{
        axios.get(`${process.env.REACT_APP_API_URL}/course/${courseName}/preregistrations` , {headers}).then((res)=>{
          setTerms(res.data)

          console.log(res.data);
        }).catch((err)=>{
            console.log(err);
        })
    
        userRole && setRole(setRole)
    
      } , [])
  return (
    <Layout>
    <div className="w-full flex flex-col">
      <div className="w-full flex justify-between px-10 py-5 border-b border-r-gray-600">
        <div className="font-semibold text-xl text-gray-700 flex"> 
            <span className="mx-0.5">دروس</span>
            <span className="mx-0.5">{terms.name}</span>
        </div>
      </div>
      {terms && terms.userIDs && <UserItems users={terms.userIDs} action={false} />}
    </div>
  </Layout>
  )
}
