...
export default function Students() {
  ...
  useEffect(()=>{
    ...
    axios.get(`${process.env.REACT_APP_API_URL}/students` , {headers}).then((res)=>{
      setTerms(res.data)
    })
    ...
    <Layout>
      <div className="w-full flex flex-col">
        <div className="w-full flex justify-between px-10 py-5 border-b border-r-gray-600">
          <h1 className="font-semibold text-xl text-gray-700">مشاهده لیست دانشجویان</h1>
        </div>
        <ListItems terms={terms} itemToAbsolutePath={`/students`}/>
      </div>
    </Layout>
  );
}
