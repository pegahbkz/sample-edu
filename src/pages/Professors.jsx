...
export default function Professors() {
  ...
  useEffect(()=>{
    ...
    axios.get(`${process.env.REACT_APP_API_URL}/professors` , {headers}).then((res)=>{
      setTerms(res.data)
    })
    ...
    <Layout>
      <div className="w-full flex flex-col">
        <div className="w-full flex justify-between px-10 py-5 border-b border-r-gray-600">
          <h1 className="font-semibold text-xl text-gray-700">مشاهده لیست اساتہد</h1>
        </div>
        <ListItems terms={terms} itemToAbsolutePath={`/professors`}/>
      </div>
    </Layout>
  );
}
